...
class Auth:
    ...
    async def _start_auth(self, mechanism: str, username: str, db_name: str) -> tuple[bytes, bytes, bytes, int]:
    ...
    async def create(self, mechanism: str, credentials: AuthCredentials) -> bytes:
        ...
        nonce, server_first, first_bare, cid = await self._start_auth(mechanism, credentials.username, credentials.db_name)
